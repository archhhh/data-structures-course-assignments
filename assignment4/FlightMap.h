...
private:
  ...
  // -------------------------------------------------------------------------
  void findRouteUtil(list<string>& ans,const string &airport1, const string &airport2,  map <string, bool> &visited,   map <string, string> &before);
  struct greater{
    bool operator()(const pair<string, double>& airport1, const pair<string, double>&airport2){
      return airport1.second >= airport2.second;
    }
  };
};
